...
Aurora follows a **modular monorepo** design with clear separation of concerns across three independently deployable services:
...
**Backoff Formulas** (implemented in [`RetryPolicy.calculateDelay()`](file:///c:/TSVV/Codity.Ai/aurora-scheduler/apps/api/src/queues/entities/retry-policy.entity.ts#L63-L78)):
...
| `payload` | `JSONB` | DEFAULT `{}` | Arbitrary JSON payload (opaque to Aurora) |
...
Aurora implements **dual authentication**:
...
The core of Aurora's reliability guarantee is the **atomic claim query**, implemented in both [`JobClaimService`](file:///c:/TSVV/Codity.Ai/aurora-scheduler/apps/api/src/jobs/job-claim.service.ts#L30-L92) and [`PollerService`](file:///c:/TSVV/Codity.Ai/aurora-scheduler/apps/worker/src/poller/poller.service.ts#L116-L155):
...
The [`HeartbeatService`](file:///c:/TSVV/Codity.Ai/aurora-scheduler/apps/worker/src/heartbeat/heartbeat.service.ts) implements dual-channel heartbeats:
...
The [`JobClaimService.reclaimStaleJobs()`](file:///c:/TSVV/Codity.Ai/aurora-scheduler/apps/api/src/jobs/job-claim.service.ts#L98-L123) method:
...
the [`PollerService.onModuleDestroy()`](file:///c:/TSVV/Codity.Ai/aurora-scheduler/apps/worker/src/poller/poller.service.ts#L65-L90) method:
...
Instead of running cron jobs directly, Aurora uses a **materializer** pattern:
...
#### Retry Policy Backoff Tests ([`retry-policy.spec.ts`](file:///c:/TSVV/Codity.Ai/aurora-scheduler/apps/api/src/queues/retry-policy.spec.ts))
...
# Run in watch mode
pnpm --filter @aurora/api run test:watch
```
...
- **User**: `demo@aurora.dev` / `demo12345`
