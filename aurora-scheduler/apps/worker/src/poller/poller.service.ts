  ...
  private async claimNextJob(): Promise<{ job: Job; execution: JobExecution } | null> {
    ...
    for (const queue of queues) {
      const result = await this.dataSource.transaction(async (em) => {
        ...
        const job = Array.isArray(rows[0]) ? rows[0][0] : rows[0];
        if (!job) return null;

        const jobId = job.id || job.job_id;
        const insertRes = await em.insert(JobExecution, {
          jobId: jobId,
        ...
        const execution = { id: insertRes.identifiers[0].id, jobId, attemptNumber: (job.attempts || 0) + 1 } as JobExecution;
